...
class CrmPhonecall(models.Model):
    ...
    def create_search_voip(self, vals):
        ...
        if not ringcentral_call_ids:
            self.create(vals)
            self.env['ringcentral.sync'].create_filter()
    ...
    def synch_data(self, record_list):
        ...
        for record in record_list:
            if record.get('id'):
                ...
                # 3/0
                if voip_ids and record.get('direction') == "Inbound":
                    vals = {
                    ...
                    voip_ids.write(vals)
                if voip_ids and record.get('direction') == "Outbound":
                    vals = {
                            'name': record.get('from').get(
                                'phoneNumber') or record.get('to').get(
                                'extensionNumber') or 'No Number Found',
                            "partner_phone": record.get("to").get(
                                "phoneNumber"),
                        }
                    voip_ids.write(vals)
                if not voip_ids:
    ...
    def create(self, values):
        ...
        if values.get("name") and values.get("type") == "in_bound":
        ...
        # if values.get("ringcentral_call_id"):
        #     if values.get("type") == "out_bound":
        #         values.update({"name": values.get("name"), "partner_phone": values.get("partner_phone")})
        #     return super(CrmPhonecall, self).create(values)
        # else:
