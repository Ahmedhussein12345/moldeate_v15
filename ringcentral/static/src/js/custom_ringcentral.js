...
//    });
    ...
    var ringcentralPanel = Widget.extend({
        ...
        render_contacts: function() {
           ...
           this.$el.find('#show_contacts').prepend(QWeb.render('ringcentral.contacts.search'))
...
//                self.$el.find('#active_calls').html(QWeb.render('ringcentral.permission.error', {}))
        ...
        contact_search: function() {
            ...
            this.$el.find('#show_contacts .nav-tabs').remove();
            this.$el.find('#show_contacts .tab-content').remove();
                                    ...
                                    if (win.document.URL.indexOf(redirectUri) != -1) {
                                        ...
                                        if ('code' in qs) {
                                            self.platform.login(qs).then(function(response) {
                                                ...
                                                .then(function(data) {
                                                    if (self.loggedin) {
                                                        self.register(data).then(function(){
                                                            self._fetchUsername();
        ...
        _fetchUsername: function(){
            ...
            return self.platform.get('/restapi/v1.0/account/~/extension/~/phone-number?usageType=DirectNumber').then(function(account_details) {
                var obj = JSON.parse(account_details.text());
                if (obj.records[0]) {
        ...
        register: function(data) {
            ...
            // console.log(":::::::::::self.usernameself.username", self.username, self.webPhone)
            
            // var methodDef;
            // var def = new Promise(function (resolve, reject) {
            //     methodDef = self.platform.get('/restapi/v1.0/account/~/extension/~/phone-number?usageType=DirectNumber').then(function(account_details) {
            //         var obj = JSON.parse(account_details.text());
            //         if (obj.records[0]) {
            //             self.username = obj.records[0].phoneNumber
            //             console.log(":::::::::yes i am here", self.username, self.webPhone)
            //         }
            //     })

            //     methodDef.then(resolve, resolve);
            // });
            // console.log(":::::::::yes final", self.username, self.webPhone)
            return self.platform.get('/restapi/v1.0/account/~').then(function(account_details) {
                var account_json = account_details.json();
                self.username = account_json.mainNumber;
